...
<div style="background-color:#FFFFFF; border:1px solid #CCC">
  <ul class="nav nav-pills nav-stacked sidebar">
    <?php
						...
						$sql='SELECT achievement_no,about FROM achievements ORDER BY achievement_id DESC';
						...
						{
							$achievement_no=$row['achievement_no'];
							$achievement_no="#".$achievement_no;
							$about=$row['about'];
							echo "<li><a href=$achievement_no>$about</a></li>";
						}
						mysql_close($conn);
					?>
  </ul>
			...
			<table class="table table-hover" style="font-family:tahoma; font-size:14px;">
				<tbody>
					<?php
						...
						mysql_select_db('vsvm');
						$sql='SELECT pic,about,content,achievement_no FROM achievements ORDER BY achievement_id DESC';
						$retval=mysql_query($sql,$conn);
						if(!$retval)
						{
							die('Could not get data: '. mysql_error());
						...
						while($row=mysql_fetch_array($retval, MYSQL_ASSOC))
						{
							...
							echo "<tr>
									<div class='row'>
									<div id=$achievement_no style='text-align:center;'>$about</div>
									</div>
								 <div class='row'>
